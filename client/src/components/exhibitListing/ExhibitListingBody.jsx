...
// tag改成li和ul！！！！
// 住址和房間測所數還沒寫
const ExhibitListingBody = ({ getUnverifiedPerListingMutation }) => {
  const { data, isSuccess } = getUnverifiedPerListingMutation;
  return (
    <div>
      {/* 標題 */}
      <div className="flex justify-start item-center gap-2">
        ...
        <h1 className="text-[20px] font-bold mt-1">{data?.name}</h1>
      </div>
      <div className="h-[2px] w-full dark:bg-gray-600 bg-gray-500 my-7"></div>

      {/* 資訊大區塊 */}
      <div className="mb-[200px] md:flex md:flex-row-reverse">
        {/* 連絡房東 */}
          ...
          <div>
            <div>
              ...
              <div className="flex flex-col gap-2">
                <span className="text-[16px] font-semibold mb-1">提供設備</span>
                <span>
                  <LuSofa className="inline-block" />
                  <p className="inline-block px-2">
                    {data?.furnished ? "附家具" : "無"}
                  </p>
                </span>
                <span>
                ...
                <span>
                  ...
                  {/* {data?.updatedAt && <span>{`更新：${data?.updatedAt}`}</span>} */}
                </span>
              </div>
            </div>
          </div>
            ...
            {data?.discountPrice ? (
              <>
                ...
                <span className="text-[16px] font-semibold mb-1">
                  ...
                  <p className="inline-block px-2">{data?.discountPrice}</p>
                </span>
              </>
            ) : (
              <>
                ...
                <span>
                  <FaRegMoneyBill1 className="inline-block" />
                  <p className="inline-block px-2">{data?.regularPrice}</p>
                </span>
              </>
            )}
            {/* </div> */}
